    ...
    return (
        <div>
            <div className="d-flex flex-column flex-md-row align-items-start justify-content-between gap-3 mb-4">
                <div>
                    <p className="text-gold text-uppercase small mb-2">Restaurant dashboard</p>
                ...
                <div className="d-flex gap-2 flex-wrap">
                    ...
                    <button className="btn btn-outline-gold" onClick={() => navigate('/reservations')}>
                        New reservation
                    </button>
                    <button className="btn btn-outline-gold" onClick={() => navigate('/orders')}>
                        Review orders
                    </button>
                </div>
            </div>
                        ...
                        <div className="card-body">
                            ...
                            <div className="list-group list-group-flush">
                                {recentOrders.map((order) => (
                                    <div key={order.id} className="list-group-item bg-transparent border-bottom border-secondary text-white">
                                        <div className="d-flex align-items-center justify-content-between">
                                            <div>
                                                <div className="fw-semibold">{order.id}</div>
                                                <div className="small text-muted">{order.customer} · {order.type}</div>
                                            </div>
                                            <span className={`badge ${order.status === 'completed' ? 'bg-success' : order.status === 'ready' ? 'bg-warning text-dark' : 'bg-secondary'}`}>
                                                {order.status}
                                            </span>
                                        </div>
                                        <div className="small text-muted mt-2">{order.items.join(', ')}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="card-body">
                            ...
                            <div className="list-group list-group-flush">
                                {upcomingReservations.map((reservation) => (
                                    <div key={reservation.id} className="list-group-item bg-transparent border-bottom border-secondary text-white">
                                        <div className="d-flex align-items-center justify-content-between">
                                            <div>
            ...
            <div className="card border-0 shadow-sm bg-black text-white mt-4">
                <div className="card-body">
                    <div className="d-flex align-items-center justify-content-between mb-3">
                        <div>
                            <h5 className="mb-1">Guest reviews</h5>
                            <p className="text-muted small mb-0">Recent sentiment from the dining floor.</p>
                        </div>
                        <span className="badge bg-gold text-dark">Feedback</span>
                    </div>
                    <div className="row g-3">
                        {reviews.slice(0, 3).map((review) => (
                            <div key={review.id} className="col-12 col-md-4">
                                <div className="p-3 rounded-4 bg-dark text-white h-100">
                                    <div className="fw-semibold mb-2">{review.name}</div>
                                    <div className="text-muted small mb-3">{Array.from({ length: review.rating }, () => '★').join('')}</div>
                                    <p className="small mb-0">{review.note}</p>
                                </div>
                            </div>
                        ))}
                    </div>
